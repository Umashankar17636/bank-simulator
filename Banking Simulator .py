...
def main_screen():
    ...
    bg_img = Image.open("money.jpg")
    bg_img = bg_img.resize((1600, 700))
    bg_photo = ImageTk.PhotoImage(bg_img)

    bg_lbl = Label(frm, image=bg_photo)
    bg_lbl.image = bg_photo          # 🔴 IMPORTANT
...
img=Image.open("GIO.jpg").resize((200,150))
tkimg=ImageTk.PhotoImage(img,master=root)

GIO_lbl=Label(root,image=tkimg)
GIO_lbl.place(relx=0,rely=0)


footer_lbl=Label(root,text="Developed by: Uma Shankar Pandey  |  📞 6203016288",font=('arial',20,'bold'),
                bg="powder blue")
